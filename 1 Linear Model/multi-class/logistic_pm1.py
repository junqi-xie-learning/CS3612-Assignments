...
def logistic_loss(X, y, beta):
    return np.sum(np.log(1 + np.exp(-y * X.transpose().dot(beta))))

def gradient(X, y, beta):
    return -X.dot(y * np.exp(-y * X.transpose().dot(beta)) / (1 + np.exp(-y * X.transpose().dot(beta))))
...
for type in types:
    ...
    for i in range(len(y_current)):
        y_current[i] = 1 if y_current[i] == type else -1
